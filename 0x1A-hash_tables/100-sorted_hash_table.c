 ...
 * Return: void
 */
void shash_table_delete(shash_table_t *new_ht)
{
unsigned long int i;
shash_node_t *next;

if (new_ht == NULL || new_ht->array == NULL || new_ht->size == 0)
return;
for (i = 0; i < new_ht->size; i++)
{
while (new_ht->array[i] != NULL)
{
next = new_ht->array[i]->next;
free(new_ht->array[i]->key);
...
free(new_ht->array);
new_ht->array = NULL;
new_ht->shead = new_ht->stail = NULL;
new_ht->size = 0;
free(new_ht);
}
